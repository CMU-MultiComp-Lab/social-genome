...
# Original Author: Sheryl Mathew, modified for Social Genome 
import json
import os
...
original_split_path = join(args.data_dir, 'videos.json')
...
def find_active_videos(ids):
    found_vids = []
    for id in tqdm(ids, desc='Downloading video data for Social Genome Dataset'):
        if id not in trims:
